...
<hr /><h2>給想轉型的人的建議</h2><h3>1. 從現有工作中找 AI 切入點</h3><p>你不需要換工作才能開始。問問自己：現在的產品有沒有哪個環節可以用 AI 優化？<strong>轉型不是一個瞬間的決定，而是一連串小實驗的累積。</strong></p><h3>2. 學對東西</h3><p>我建議的學習優先級：</p><ol><li><strong>Prompt Engineering</strong>（投報率最高，馬上可以用）</li></ol><ol><li><strong>AI 產品設計 Pattern</strong>（Human-in-the-loop、Progressive Disclosure）</li></ol><ol><li><strong>基礎 ML 概念</strong>（Training、Inference、Evaluation 的基本邏輯）</li></ol><ol><li><strong>數據基礎</strong>（SQL 是加分項，但更重要的是建立數據直覺）</li></ol><h3>3. 大量使用 AI 工具</h3><p>不要只是用 ChatGPT 幫你寫信。試著用它做 User Research 分析、寫 PRD 初稿、產生測試案例。<strong>當你自己是 AI 的深度用戶，你才能真正理解 AI 產品的痛點和機會。</strong></p><h3>4. 建立跨領域的語言能力</h3><p>多跟 AI 工程師、Data Scientist 聊天。一開始聽不懂很正常，不要怕問笨問題。</p><h3>5. 接受「不完美」的產品觀</h3><p><strong>不是追求 100% 正確，而是設計一個「就算 AI 出錯也不會造成災難」的產品體驗。</strong> 💡</p><hr /><h2>寫在最後</h2><p>回頭看這段轉型的路，我最慶幸的一件事是：<strong>我沒有等到「準備好」才開始。</strong></p><p>如果你現在也站在那個不確定的路口，先從一個小實驗開始。找到你現在工作裡跟 AI 最近的那個接觸點，動手去做。</p><p>AI 產品經理這個角色還在被定義中，這代表你有機會用自己的方式去塑造它。</p><p>如果你也正在經歷類似的轉型，或是對 AI PM 的日常有任何好奇，歡迎留言跟我聊聊——我很想知道，你的轉型故事是什麼？ 🚀</p>`,
    url: '',
  },
  {
