...
import { generateCheckMacValue, generateUserToken } from './lib/ecpay';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // CustomField1 = create-order 寫入的付款人 user_id，已隨 body 一起通過上面的 CheckMacValue 驗證，可信。
  const payerUserId = body.CustomField1 || '';
  const token = verified ? generateUserToken(tradeNo, payerUserId, hashKey) : '';
